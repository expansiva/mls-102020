...
export function deriveNs4E8Skeleton(sources: Ns4E8Sources, reviewRound = 1): Ns4E8SkeletonReview {
  ...
  const workspaces = [...clusters.entries()].map(([key, entries]) => {
    ...
    const pageContext = uniqueContexts(entries.flatMap(entry => entry.requires.filter(context => !entries.some(other => other.provides.some(provided => provided.contextId === context.contextId)))));
  ...
  const headerLinks = workspaces.filter(workspace => workspace.kind === 'hub' || !workspace.pageContext.length).map(workspace => workspace.workspaceId);
  const sections = sources.journeys.features.map(feature => ({
    ...
    workspaceIds: unique(workspaces.filter(workspace => workspace.featureRefs.includes(feature.featureId)).map(workspace => workspace.workspaceId)),
  })).filter(section => section.workspaceIds.length);
  const landings = sources.access.profiles.flatMap(profile => {
    const target = workspaces.find(workspace => workspace.profileRefs.includes(profile.profileId) && (!workspace.pageContext.length || workspace.kind === 'hub'));
  ...
  return {
    ...
    userLanguage: sources.journeys.userLanguage, title: 'Workspaces', reviewRound, hubRanking: ranking, workspaces,
    menu: { headerLinks, sections, landings }, edges, contextCatalog: uniqueContexts([...contexts.values()]), changeSummary: [],
  };
}
...
export function normalizeNs4E8Skeleton(value: unknown, fallbackModule = ''): Ns4E8SkeletonReview {
  ...
  return {
    ...
    contextCatalog: array(root.contextCatalog).map(normalizeContext), ...(text(root.skeletonHash) ? { skeletonHash: text(root.skeletonHash) } : {}), changeSummary: strings(root.changeSummary),
  };
}

export function overlayNs4E8Presentation(derived: Ns4E8SkeletonReview, proposed: unknown): Ns4E8SkeletonReview {
  const candidate = normalizeNs4E8Skeleton(proposed, derived.moduleName);
  ...
  const labels = new Map(candidate.menu.sections.map(section => [section.featureRef, section]));
...
function deriveScenarios(entries: ReturnType<typeof collectSteps>, isHub: boolean, sources: Ns4E8Sources): Ns4WorkspaceScenario[] {
  ...
  if (isHub) {
    ...
    scenarios.unshift({ scenarioId: 'collection', kind: 'collection', title: 'Collection', description: 'Portfolio collection and selection.', stepRefs: query ? [query.stepRef] : [], useCaseIds: query?.useCaseId ? [query.useCaseId] : [], authorityRefs: query?.authorityRefs || [] });
    scenarios.unshift({ scenarioId: 'record', kind: 'record', title: 'Record', description: 'Selected record dashboard and internal menu.', stepRefs: [], useCaseIds: [], authorityRefs: [] });
  }
  ...
  if (entries.some(entry => entry.authorityRefs.length && (workflowEntities.has(entry.primaryEntity)
    ...
    scenarios.push({ scenarioId: 'queue', kind: 'queue', title: 'Decision queue', description: 'Reachable pending decisions.', stepRefs: [], useCaseIds: [], authorityRefs: unique(entries.flatMap(entry => entry.authorityRefs)) });
  }
  return scenarios;
}
