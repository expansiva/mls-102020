...
export class WidgetNs4Workspaces102020 extends StateLitElement {
  ...
  render() {
    ...
    return html`<section class="ns4-workspaces">
      ...
      <label>Changes<textarea .value=${this.adjustment} @input=${(event: Event) => { this.adjustment = (event.target as HTMLTextAreaElement).value; }}></textarea></label>
      <footer><button @click=${() => this.submit('requestChanges')} ?disabled=${!this.adjustment.trim()}>Request changes</button><button @click=${() => this.submit('approve')} ?disabled=${Boolean(this.adjustment.trim())}>Approve map</button></footer>
    </section>`;
  }
  private submit(action: Ns4E8ReviewEvent['action']): void {
    if (!this.value || action === 'cancel') return;
