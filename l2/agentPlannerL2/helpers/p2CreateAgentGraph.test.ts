/// <mls fileReference="_102020_/l2/agentPlannerL2/helpers/p2CreateAgentGraph.test.ts" enhancement="_blank"/>
...
const MLS_BASE = path.resolve(P2_ROOT, '../../..');
const ENTRY = path.join(P2_ROOT, 'agentPlannerL2.ts');
...
function resolveInProject(fromFile: string, spec: string): string | null {
  ...
  if (spec.startsWith('/_102020_/')) {
    candidate = path.join(MLS_BASE, 'mls-102020', spec.replace(/^\/_102020_\//, '').replace(/\.js$/, '.ts'));
  } else if (spec.startsWith('.')) {
    candidate = path.resolve(path.dirname(fromFile), spec.replace(/\.js$/, '.ts'));
  }
  if (!candidate) return null;
  if (!existsSync(candidate)) return null;
  const rel = path.relative(P2_ROOT, candidate);
...
function relP2(file: string): string {
  return path.relative(P2_ROOT, file).replace(/\\/g, '/');
}
...
function walkCreateAgentGraph(entry = ENTRY): { files: string[]; offences: Offence[] } {
  ...
  while (queue.length) {
    ...
    for (const spec of staticImportSpecifiers(source)) {
      ...
      if (reason) offences.push({ file: relP2(file), reason });
      const next = resolveInProject(file, spec);
      if (next) queue.push(next);
    }
    for (const ident of globalOffences(source)) {
      offences.push({ file: relP2(file), reason: `prompt-hook graph uses ${ident}` });
    }
  }
...
void test('createAgent static graph stays inside agentPlannerL2 and the three solution modules', () => {
  assert.equal(existsSync(ENTRY), true, `missing ${ENTRY}`);
  const { files, offences } = walkCreateAgentGraph();
  const rels = files.map(relP2);
  ...
  assert.ok(rels.includes('steps/menu20/agentP2Menu.ts'), 'menu20 must be in the graph');
...
void test('guard goes red on a forbidden 102035 import and on window', () => {
  const poisoned = [
    ...
    'export function beforeP2EntryPromptStep() { return window.location; }',
  ].join('\n');
