...
L2 planner. Reads a finished l4 module and the oldest message in `pool/l2`, then (later specs)
...
This spec (`p2_01`) delivers the skeleton and `entry10` only. Steps 20–50 are declared in
`docs/flow.json` as `waiting`.
...
`entry10` is deterministic. A declared step without a hook stops the run with
