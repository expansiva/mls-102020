...
- One tree per module, one file per device (`web` today). Nodes are `hub`, `page` or `group`. A page has `organisms[]` (9 kinds, including `inbox` and `alerts`); a hub has `context` + `text` + `children`. Home organisms (`summary`/`highlights`, `alerts`, `inbox`) only when "what this actor must see, beyond journeys" has a source for that kind; otherwise the actor has no home and enters at their first work page. Every node has `action`: `new` | `change` | `keep` | `remove`. Today only `new` is emitted (`meta.removed` is always `[]`) because nothing is built in l2 yet. When the manifesto exists, `action` becomes a structural diff against it.
