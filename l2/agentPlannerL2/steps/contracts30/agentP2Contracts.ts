/// <mls fileReference="_102020_/l2/agentPlannerL2/steps/contracts30/agentP2Contracts.ts" enhancement="_blank"/>
...
import {
  formatP2ContractGate,
  validateP2Contracts,
} from '/_102020_/l2/agentPlannerL2/steps/contracts30/gate.js';

const MAX_REPAIRS = 2;
const MAX_TRANSPORT_RETRIES = 1;

interface ContractsArgs {
  planId: string;
  moduleName: string;
...
export function buildP2ContractsHumanPrompt(input: {
  sources: P2L4Sources;
  workspaces: P2WorkspacesDraft;
  gateFeedback?: string;
  previousDraft?: unknown;
}): string {
  const catalog = collectP2FieldCatalog(input.sources);
  const slots = collectP2CallSlots(input.workspaces, input.sources);
  const fieldsByEntity = new Map<string, typeof catalog>();
  ...
  for (const [entityId, fields] of [...fieldsByEntity.entries()].sort((left, right) => left[0].localeCompare(right[0]))) {
    fieldLines.push(`### ${entityId}`);
    for (const field of fields) {
      ...
      fieldLines.push(`- ${field.path} (${flags.join(', ')})`);
    }
  }
  return [
    `## userLanguage`,
    ...
    '',
    '## Call slots (deterministic; one call per slot)',
    JSON.stringify(slots, null, 2),
    '',
    '## Ontology field catalog (cite these paths only)',
...
export async function beforeP2ContractsPromptStep(
  _agent: IAgentMeta,
  ...
  if (!context.task) throw new Error('[agentPlannerL2:contracts30] task invalid');
  let moduleName = '';
  try {
    ...
    const [skill, prompt, schema, previous] = await Promise.all([
      readP2AgentText('skills', 'contracts', '.md'),
      readP2AgentText('steps/contracts30', 'prompt', '.md'),
      readJson<Record<string, unknown>>(p2AgentFile('schemas', 'contracts.schema', '.json')),
      moduleName ? readJson(p2DraftFile(moduleName, 'contracts30')) : Promise.resolve(null),
    ]);
    if (!schema) throw new Error('contracts.schema.json is missing.');
    const tool = buildP2ContractsTool(schema);
    const humanPrompt = buildP2ContractsHumanPrompt({
      sources,
      workspaces,
...
export async function afterP2ContractsPromptStep(
  _agent: IAgentMeta,
  context: mls.msg.ExecutionContext,
  parentStep: mls.msg.AIAgentStep,
  step: mls.msg.AIAgentStep,
  ...
  try {
    ...
    if (!isRecord(payload)) {
      const failure = readPromptFailure(step, 'contracts30 returned no usable contracts artifact.');
      if (parsed.transportAttempt < MAX_TRANSPORT_RETRIES) {
        return [
          addStep(context, mutationParent, createP2RetryStep('contracts30', moduleName, 'transport', parsed.transportAttempt + 1)),
          updateStatus(context, mutationParent, step, hookSequential, 'completed', `contracts30 transport retry ${parsed.transportAttempt + 1} scheduled: ${failure}`),
        ];
      }
    ...
    const workspaces = await readJson<P2WorkspacesDraft>(p2DraftFile(moduleName, 'workspaces20'));
    if (!workspaces) throw new Error('workspaces20-draft.json is missing; workspaces20 must run first.');
    const draft = normalizeP2ContractsPayload(payload, moduleName);
    let pipeline = await requirePipeline(moduleName);
    pipeline = await writeStepState(pipeline, {
      status: 'running',
      updatedAt: new Date().toISOString(),
    });
    const draftPath = await writeJson(p2DraftFile(moduleName, 'contracts30'), draft);
    const gate = validateP2Contracts(draft, workspaces, sources);
    if (!gate.ok) {
      const feedback = formatP2ContractGate(gate.issues);
      if (parsed.repairAttempt < MAX_REPAIRS) {
        return [
          addStep(context, mutationParent, createP2RetryStep('contracts30', moduleName, 'repair', parsed.repairAttempt + 1, { gateFeedback: feedback })),
          updateStatus(context, mutationParent, step, hookSequential, 'completed', `contracts30 gate scheduled repair ${parsed.repairAttempt + 1}.`),
        ];
      }
    ...
    for (const workspace of draft.workspaces) {
      const source = emitP2ContractDefs({
        project,
        moduleName,
      ...
      artifactPaths.push(await writeP2Text(p2ContractFile(moduleName, workspace.workspaceId), source));
    }
    ...
    return [
      ...
      updateStatus(context, mutationParent, step, hookSequential, 'completed', `contracts30 approved: ${artifactPaths.join(', ')}`),
    ];
  } catch (error) {
    const message = errorMessage(error);
    await recordFailure(moduleName, message);
...
export async function writeP2Text(fileInfo: Ns5FileInfo, content: string): Promise<string> {
  assertShortName(fileInfo.shortName);
  const key = mls.stor.getKeyToFile(fileInfo);
  let file = mls.stor.files[key];
  if (!file) {
    const stor = mls.stor as { addOrUpdateFile?: unknown };
    if (typeof stor.addOrUpdateFile === 'function') {
      file = await createStorFile({ ...fileInfo, source: content }, false, false, false);
    } else {
      file = { ...fileInfo, status: 'changed', versionRef: '1' } as mls.stor.IFileInfo;
      mls.stor.files[key] = file;
    }
...
async function writeStepState(
  ...
  next: P2PipelineState['steps']['contracts30'] & { status: 'running' | 'approved' | 'failed'; updatedAt: string },
): Promise<P2PipelineState> {
  const updated = markP2Step(pipeline, 'contracts30', next);
...
async function recordFailure(moduleName: string, error: string): Promise<void> {
  ...
  try {
    const pipeline = await readP2Pipeline(moduleName);
    if (!pipeline) return;
    await writeJson(p2PipelineFile(moduleName), markP2Step(pipeline, 'contracts30', {
      status: 'failed',
      updatedAt: new Date().toISOString(),
      error,
    }));
...
function resolveArgs(context: mls.msg.ExecutionContext, value: unknown): ContractsArgs {
  const root = parseRecord(value);
  const moduleName = text(root.moduleName) || memoryString(context, 'moduleName');
  return {
    planId: text(root.planId) || 'contracts30',
    moduleName,
...
function doneAnchor(
  ...
  return addStep(context, parentStep, {
    ...
    result: JSON.stringify({
      ...
      nextStep: P2_STEP_IDS[3],
    }),
    planning: { planId: 'contracts30-done', dependsOn: [], executionMode: 'manual_later', executionHost: 'client' },
...
P2_STEP_HOOKS.contracts30 = {
  beforePromptStep: beforeP2ContractsPromptStep,
  afterPromptStep: afterP2ContractsPromptStep,
};
