/// <mls fileReference="_102020_/l2/agentNewSolution/steps/e3-ontology/gate.test.ts" enhancement="_blank"/>
...
function validEntity(): NsE3EntityArtifact {
...
void test('e3 entity gate rejects an entity outside the model and kind drift', async () => {
  ...
  const ghostGate = await runNsGate({
    stepId: 'e3-ontology',
    schema: entitySchema,
    artifact: entity,
    validate: item => validateE3EntityInvariants(item, { model: validModel() }),
  ...
  drift.kind = 'event';
  const driftGate = await runNsGate({
    stepId: 'e3-ontology',
    schema: entitySchema,
    artifact: drift,
    validate: item => validateE3EntityInvariants(item, { model: validModel() }),
