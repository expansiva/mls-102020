/// <mls fileReference="_102020_/l2/agentNewSolution/steps/e1-draft/gate.ts" enhancement="_blank"/>
...
export const E1_DRAFT_SCHEMA_VERSION = '2026-07-05-ns-e1-v1';

export interface NsE1Actor {
  actorId: string;
  name: string;
  assumption: string;
}

export interface NsE1OpenQuestion {
  questionId: string;
  question: string;
...
export interface NsE1DraftArtifact {
  ...
  openQuestions: NsE1OpenQuestion[];
  assumptions: string[];
  createdAt: string;
}
...
export function prepareE1DraftArtifact(input: unknown, options: E1GateOptions = {}): NsE1DraftArtifact {
  ...
  return {
    ...
    openQuestions: readArray(record.openQuestions).map((item, index) => {
      ...
      const normalized: NsE1OpenQuestion = {
        questionId: uniqueNsId(question.questionId || question.question, usedQuestions, `question${index + 1}`),
...
export function renderE1DraftMarkdown(artifact: NsE1DraftArtifact): string {
  const lines = [
