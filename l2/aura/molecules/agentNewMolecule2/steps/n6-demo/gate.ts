 ...
 */
/**
 * A `{{playground.*}}` binding written as the CONTENT of a named slot — `<Label>{{playground.basic.label}}</Label>`.
 *
 * ⚠️ MEASURED 2026-08-18. Bindings are resolved on ATTRIBUTES. Inside slot content the token is plain
 * text, so the page shows the literal `{{playground.basic.label}}` on screen. Across the 196 pages of
 * the three molecule projects the count is **zero** — the library always writes slot content as literal
 * text — and a page just regenerated by route E had **12**.
 *
 * The prompts already say "slot content goes in the markup, never in the state", which is the same rule
 * stated as prose. Prose asks, code imposes.
 */
export function findSlotBindings(html: string): string[] {
  const out: string[] = [];
  for (const m of (html || '').matchAll(/<([A-Z][A-Za-z0-9]*)\s*>\s*(\{\{\s*playground\.[^}]*\}\})/g)) {
    out.push(`<${m[1]}>${m[2]}`);
  }
  return out;
}
...
export function demoStateIssues(html: string, examples: MoleculeDemoExample[]): NmGateIssue[] {
  ...
  if (orphans.length) {
    issues.push({
      code: 'state_binding',
      message: `these bindings have no matching example state and would render empty: ${orphans.map(key => `playground.${key}`).join(', ')}. The fix is to DECLARE the missing entries in \`examples\` (one per bound property, named 'playground.<key>.<property>') — never to delete bindings, cards or controls to make this message go away`,
    });
  }
  return issues;
}
...
export function runNm2DemoGate(
  ...
  const slotBindings = findSlotBindings(content);
  if (slotBindings.length) {
    issues.push({
      code: 'slot_binding',
      message: `slot content is bound to the state (${slotBindings.slice(0, 3).join(', ')}) and bindings only resolve on ATTRIBUTES — inside slot content the token is plain text, so the page shows the literal {{playground...}} on screen. Write the slot content as literal text: <Label>Copy</Label>`,
    });
  }
