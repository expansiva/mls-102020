...
import { compileStorTs, nmDestProject, NmFileInfo, readJsonArtifact, readStorText, writeJsonArtifact, writeStorTextAtomic, toDisplayPath } from '/_102020_/l2/aura/molecules/agentNewMolecule2/helpers/nmFs.js';
...
async function beforePromptStep(
  ...
  const project = input.projectTarget || nmDestProject();
  ...
  for (const group of catalogGroups) {
    ...
    const shortTags = syExtractMoleculeShortTags(await readStorText(at(nmGroupDefsFile(folder)), false));
    if (!shortTags.length) {
  ...
  await writeStorTextAtomic(syProjectSkillFile(project), skillText, true);
  ...
  const skillCompile = await compileStorTs(syProjectSkillFile(project), skillText);
  // skill.ts is imported BY NAME by the catalog consumer, so it needs to be fetchable, not just valid.
  const skillCache = await syPublishToCache(syProjectSkillFile(project));
  ...
  const artifact: SyProjectArtifact = {
    ...
    moleculeCount,
    skillFile: toDisplayPath(syProjectSkillFile(project)),
  };
