/// <mls fileReference="_102020_/l2/aura/studio/studioEditor.ts" enhancement="_blank" />
...
import {
  ...
  readSharedKeyMap,
  type TextOrigin,
} from '/_102020_/l2/aura/studio/studioTextEdit.js';
import {
  CLASS_PICKER_TAG,
...
import {
  ...
  setEditHistory,
  setEditSelection,
  type IAuraEditSelection,
} from '/_102020_/l2/aura/helpers/auraState.js';
import { EditHistory } from '/_102020_/l2/aura/studio/studioEditHistory.js';
...
} from '/_102020_/l2/aura/studio/studioEditTarget.js';

export type StudioEditMode = 'off' | 'select' | 'text' | 'inspect';

// What the editor tells the rest of the Studio is published on `aura.edit` (auraState), not through
// callbacks. There used to be three hooks here — onTarget, onEdited, onSelect — and in the whole
// lifetime of this editor nobody ever implemented one: the only owner constructs it with no events at
// all. A projection anyone can subscribe to is what that was reaching for.
...
export class StudioEditor {
  ...
  public async refreshTarget(): Promise<void> {
    ...
    if (result.ok) {
      ...
      this.statusText = '';
      this.publishSelection(this.classPanel?.el ?? null);
    } else {
      this.target = null;
  ...
  private async applyTextEditToSource(
    ...
    this.publishEdited(editTargetFile);
    return { ok: true, what };
  }
  ...
  /**
   * Publishes what is selected, for whoever wants to act on it (`aura.edit.selection`).
   *
   * DATA only — the element itself never leaves this class. The state manager keeps every value it is
   * handed in a 10.000-entry log, so an HTMLElement parked there would outlive the page it belongs to.
   *
   * It is called from the same places the old `onSelect`/`onTarget` hooks were, plus after every
   * write: the literal is part of the projection, and an edit changes it.
   */
  private publishSelection(el: HTMLElement | null): void {
    if (!el) {
    ...
    const state = this.classPanel?.el === el ? this.classPanel : null;
    const file = state?.file;
    const selection: IAuraEditSelection = {
      tag: el.tagName.toLowerCase(),
      file: file ? { project: file.project, shortName: file.shortName, folder: file.folder } : null,
  ...
  /** How deep the stack is and what the next step would be — the three answers others ask for. */
  private publishHistory(): void {
    setEditHistory({
      ...this.history.depth,
      nextUndo: this.history.peekUndo()?.what ?? '',
      nextRedo: this.history.peekRedo()?.what ?? '',
    });
  }

  /** A file now has a local edit: "there is something to save", never "it is saved". */
  private publishEdited(file: IStudioEditTarget): void {
    addEditDirty(`${file.project}:${file.folder}/${file.shortName}`);
    this.publishSelection(this.selectedEl);
  }
