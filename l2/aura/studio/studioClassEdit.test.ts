...
// ── Layers: the responsive and the state layer (TASK-102033-picker-variants) ─────────────────────

const layer = (breakpoint: LayerBreakpoint, state: LayerState = 'base'): ILayer => ({ breakpoint, state });

/** The rows of a layer, as `raw` plus a mark for what is inherited. */
function rowsOf(literal: string, at: ILayer): string[] {
  return layerRows(literal, at).map((row) => (row.inherited ? `~${row.token.raw}` : row.token.raw));
}

test('the prefix is written in ONE order: breakpoint, then state', () => {
  // Both orders work in Tailwind, so this is a convention — and the convention is what keeps the file
  // predictable for whoever reads it next.
  assert.equal(layerPrefix(BASE_LAYER), '');
  assert.equal(layerPrefix(layer('md')), 'md:');
  assert.equal(layerPrefix(layer('base', 'hover')), 'hover:');
  assert.equal(layerPrefix(layer('md', 'hover')), 'md:hover:');

  assert.equal(layerClass('p-4', layer('md', 'hover')), 'md:hover:p-4');
  // Re-prefixing replaces OUR variants and keeps the foreign ones — the animation tab's
  // `motion-safe:` must not vanish because someone clicked a chip.
  assert.equal(layerClass('sm:hover:animate-spin', layer('lg')), 'lg:animate-spin');
  assert.equal(layerClass('motion-safe:animate-spin', layer('md')), 'md:motion-safe:animate-spin');
  assert.equal(layerClass('md:motion-safe:animate-spin', BASE_LAYER), 'motion-safe:animate-spin');
});

test('a token knows its layer, and a foreign variant has none', () => {
  const at = (raw: string) => layerOf(splitUtilities(raw)[0]);

  assert.deepEqual(at('p-3'), BASE_LAYER);
  assert.deepEqual(at('md:p-3'), layer('md'));
  assert.deepEqual(at('hover:bg-red-500'), layer('base', 'hover'));
  assert.deepEqual(at('md:hover:p-6'), layer('md', 'hover'));
  assert.deepEqual(at('hover:md:p-6'), layer('md', 'hover'), 'the order it was written in does not matter');

  // Not a layer this picker offers to switch to: it keeps reading them, it just never writes one.
  assert.equal(at('group-hover:p-3'), null);
  assert.equal(at('motion-safe:animate-spin'), null);
  assert.equal(at('starting:opacity-0'), null);
  // Two of the same dimension is not a layer either — it is a mistake, and offering to deepen it
  // would be offering to make it worse.
  assert.equal(at('sm:md:p-3'), null);
});

test('a deeper layer INHERITS what it does not define, and says where from', () => {
  const literal = 'p-3 text-sm md:p-6';

  assert.deepEqual(rowsOf(literal, BASE_LAYER), ['p-3', 'text-sm']);
  // In `md` the padding is the layer's own and the text size is inherited from base.
  assert.deepEqual(rowsOf(literal, layer('md')), ['~text-sm', 'md:p-6']);
  // And in `lg`, BOTH are inherited — the padding from `md`, which is the deepest applier.
  assert.deepEqual(rowsOf(literal, layer('lg')), ['~text-sm', '~md:p-6']);
  // `sm` is shallower than `md`: the override does not apply there.
  assert.deepEqual(rowsOf(literal, layer('sm')), ['~p-3', '~text-sm']);

  const from = layerRows(literal, layer('lg')).find((row) => row.token.raw === 'md:p-6');
  assert.deepEqual(from?.from, layer('md'), 'the row carries the layer it came from');
});

test('a state layer inherits from the stateless one, and its own definition wins', () => {
  const literal = 'bg-white text-sm hover:bg-gray-100 md:hover:bg-gray-200';

  assert.deepEqual(rowsOf(literal, BASE_LAYER), ['bg-white', 'text-sm'], 'hover does not apply with no state');
  // The background is the hover layer's OWN, so base's is not shown for it — a family the layer
  // defines is not also inherited. The text size, which hover does not touch, is.
  assert.deepEqual(rowsOf(literal, layer('base', 'hover')), ['~text-sm', 'hover:bg-gray-100']);
  // In `md:hover` its own definition wins over the shallower hover.
  assert.deepEqual(rowsOf(literal, layer('md', 'hover')), ['~text-sm', 'md:hover:bg-gray-200']);
  // In `md` with no state, the hover rules have no say at all.
  assert.deepEqual(rowsOf(literal, layer('md')), ['~bg-white', '~text-sm']);
});

test('choosing on an inherited row CREATES the override; on its own row it replaces', () => {
  const literal = 'p-3 text-sm';
  const rows = layerRows(literal, layer('md'));
  const padding = rows.find((row) => row.token.family === 'p');
  assert.ok(padding?.inherited);

  // The inherited case is the one that surprises: the value shown is `p-3`, and choosing `p-6` must
  // not touch the base — it writes a new class for this layer.
  assert.equal(applyInLayer(literal, padding, 'p-6', layer('md')), 'p-3 text-sm md:p-6');
  // Choosing the SAME value is still an edit here: `md:p-3` is what pins the layer.
  assert.equal(applyInLayer(literal, padding, 'p-3', layer('md')), 'p-3 text-sm md:p-3');

  // And once the layer owns it, the next choice replaces in place.
  const owned = layerRows('p-3 md:p-6', layer('md')).find((row) => row.token.raw === 'md:p-6');
  assert.ok(owned && !owned.inherited);
  assert.equal(applyInLayer('p-3 md:p-6', owned, 'md:p-8', layer('md')), 'p-3 md:p-8');
  // The option may arrive with the variants of the row it was composed from; the layer decides.
  assert.equal(applyInLayer('p-3 md:p-6', owned, 'p-8', layer('md')), 'p-3 md:p-8');
});

test('removing an override brings the inheritance back, and an inherited row refuses', () => {
  const literal = 'p-3 md:p-6';
  const owned = layerRows(literal, layer('md')).find((row) => !row.inherited);
  assert.ok(owned);

  const without = removeInLayer(literal, owned);
  assert.equal(without, 'p-3');
  assert.deepEqual(rowsOf(without, layer('md')), ['~p-3'], 'the row is back to the inherited value');

  // An inherited row has nothing to remove here: taking `p-3` out would be editing the base layer
  // from inside `md`.
  const borrowed = layerRows(without, layer('md'))[0];
  assert.ok(borrowed.inherited);
  assert.equal(removeInLayer(without, borrowed), without);
});

test('every token the layer OWNS is a row, duplicates included', () => {
  // The panel shows two padding rows for `p-3 p-4` today; a rule that kept only the cascade winner
  // would make one of them impossible to see or to remove.
  assert.deepEqual(rowsOf('p-3 p-4', BASE_LAYER), ['p-3', 'p-4']);
  assert.deepEqual(rowsOf('md:p-3 md:p-4', layer('md')), ['md:p-3', 'md:p-4']);
  // Inheritance, on the other hand, fills only what is missing: one row, the deepest applier.
  assert.deepEqual(rowsOf('p-3 p-4', layer('md')), ['~p-4']);
});

test('a foreign variant is a row in the base layer and nowhere else', () => {
  // It has no layer to switch to, so base is where it stays reachable — which is also where the
  // panel has always shown it.
  const literal = 'p-3 motion-safe:animate-spin group-hover:text-white';

  assert.deepEqual(rowsOf(literal, BASE_LAYER), ['p-3', 'motion-safe:animate-spin', 'group-hover:text-white']);
  const inMd = layerRows(literal, layer('md'));
  assert.deepEqual(inMd.filter((row) => row.from === null).map((row) => row.token.raw),
    ['motion-safe:animate-spin', 'group-hover:text-white'],
    'they still come back — the layer preview needs them, the panel filters them out');

  // Choosing on a foreign row never adds a layer prefix: it is edited where it is.
  const foreign = layerRows(literal, BASE_LAYER).find((row) => row.from === null);
  assert.ok(foreign);
  assert.equal(
    applyInLayer(literal, foreign, 'motion-safe:animate-pulse', layer('md')),
    'p-3 motion-safe:animate-pulse group-hover:text-white',
  );
});

test('the simulation is the layer as if it were the only one', () => {
  // Removing the shallower class is not optional: the class attribute has no order of its own, so
  // `p-4` next to an unprefixed `p-2` would show whichever Tailwind emitted last — the wrong one.
  assert.equal(simulateLayer('p-4 md:p-2', layer('md')), 'p-2');
  assert.equal(simulateLayer('p-3 text-sm md:p-6', layer('md')), 'text-sm p-6');
  assert.equal(simulateLayer('bg-white hover:bg-gray-100', layer('base', 'hover')), 'bg-gray-100');
  // Base is itself: nothing to simulate.
  assert.equal(simulateLayer('p-3 md:p-6', BASE_LAYER), 'p-3');
  // Foreign variants travel untouched — they are real classes on the element.
  assert.equal(
    simulateLayer('p-3 md:p-6 motion-safe:animate-spin', layer('md')),
    'p-6 motion-safe:animate-spin',
  );
});

test('nothing on the element is unreachable: every token is a row in some layer', () => {
  // The invariant the whole layer switch rests on. If a class can be written and never shown, the
  // panel is lying about what the element has.
  const literals = [
    'p-3 md:p-6 lg:p-8 hover:bg-gray-100 md:hover:bg-gray-200',
    'text-sm sm:text-base xl:text-lg disabled:opacity-50',
    'p-3 motion-safe:animate-spin starting:opacity-0 group-hover:text-white',
    'flex md:grid md:grid-cols-2 gap-4',
  ];
  const layers: ILayer[] = [];
  for (const breakpoint of LAYER_BREAKPOINTS) {
    for (const state of LAYER_STATES) layers.push({ breakpoint, state });
  }

  for (const literal of literals) {
    for (const token of splitUtilities(literal)) {
      const shown = layers.some((at) => layerRows(literal, at).some((row) => row.token.raw === token.raw));
      assert.ok(shown, `${token.raw} of "${literal}" is in no layer`);
    }
    // And no layer shows the same token twice.
    for (const at of layers) {
      const raws = layerRows(literal, at).map((row) => row.token.raw);
      assert.deepEqual([...new Set(raws)].length, raws.length, `${literal} @ ${layerPrefix(at)}`);
    }
  }
});

test('the classes tab writes through the LAYER, and nowhere else', () => {
  // The guard that keeps the feature honest: if one chip still called `replaceUtility` directly, that
  // chip would silently edit the base layer while the selector said `md`.
  ...
  for (const bare of ['replaceUtility(', 'removeUtility(']) {
    assert.equal(code.some((line) => line.includes(bare)), false, `${bare} bypasses the layer`);
  }
  // `addUtility` survives for the "+", and every call of it goes through `layerClass`.
  for (const line of code.filter((entry) => entry.includes('addUtility('))) {
    assert.ok(line.includes('layerClass('), line);
  }
  // The three writes of the tab, each through the layer.
  for (const call of ['applyInLayer(this.literal, row, option, this.layer)', 'removeInLayer(this.literal, row)']) {
    assert.ok(panel.includes(call), call);
  }

  // `dark:` is not offered, and this is the line that would tempt someone: the design system's roles
  // already swap in its own dark block, so a colour never needs the prefix.
  assert.equal((LAYER_BREAKPOINTS as readonly string[]).includes('dark'), false);
  assert.equal((LAYER_STATES as readonly string[]).includes('dark'), false);
  assert.equal(panel.includes("'dark'"), false, 'no dark layer in the panel either');

  // The preview is a preview: the panel asks for it with `picker-preview`, never with `picker-apply`.
  const preview = panel.slice(panel.indexOf('private showLayerPreview'), panel.indexOf('render() {'));
  assert.ok(preview.includes('simulateLayer('), 'the simulation');
  assert.ok(preview.includes('this.emitPreview('), 'as a preview');
  assert.equal(preview.includes('applyLiteral'), false, 'and never as a write');

  // The inherited marker is rendered, because a value that reads as an override is risk 1 of the task.
  assert.ok(panel.includes("t('panel.inherited'"), 'the row says where the value comes from');
  assert.ok(panel.includes("t('panel.removeInherited'"), 'and why there is nothing to remove');
});
