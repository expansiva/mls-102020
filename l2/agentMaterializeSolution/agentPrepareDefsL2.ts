...
import { buildModuleTs, buildRouterTs, buildPersistenceTs, buildConfig } from '/_102020_/l2/agentMaterializeSolution/templateMaterialize.js';
...
async function afterPromptStep(
  ...
  try {
    ...
    if (!payload) throw new Error('[agentPrepareDefsL2] missing payload');
    ...
    const project = mls.actualProject || 0;
    const projectJson = await readProjectJson();
    if (!projectJson) throw new Error('[agentPrepareDefsL2] project.json unavailable');
...
function buildHumanPrompt(
  summaries: Array<{ moduleName: string; l2Count: number; l2Files: string[] }>,
): string {
