 ...
 * the stor itself. Each wave of each module is one sequential `agentCfeMaterializePhase` step — the
 * same host agentCfeMaterializeL2 uses — which opens the `agentCfeMaterializeGen` fan-out under
 * itself, verifies the artifacts on disk and runs the repair rounds.
 */

export type SpecWave = 'shared' | 'pages';

/** shared -> pages, the order LAYER_RANK already encodes in cfeMaterializeCore. */
export const WAVE_ORDER: readonly SpecWave[] = ['shared', 'pages'];
...
/**
 * Folders whose level-2 `.defs.ts` this agent does NOT materialize, by LAST segment of `folder`.
 * Contracts are out by decision (23/09/2026): they are generated by another path, and sweeping
 * them here made the 102047 run pick up the 6 defs of `agendaClinica/web/contracts`.
 */
export const IGNORED_LAST_SEGMENTS: readonly string[] = ['contracts'];
...
export function createAgent(): IAgentAsync {
  return {
    ...
    agentDescription: 'Materializa em lote os .defs.ts de level 2 do projeto atual, em duas ondas: shared e paginas. Contratos sao ignorados.',
    visibility: 'public',
 ...
 *
 * `null` means IGNORED — today only `contracts`. Everything that is not `shared` is a page, which
 * is what keeps both `web/desktop/pageNN` and `web/mobile/pageNN` in the pages wave (102047 has
 * both).
 */
export function waveOf(folder: string): SpecWave | null {
  const last = String(folder || '').split('/').filter(Boolean).pop() ?? '';
  if (IGNORED_LAST_SEGMENTS.includes(last)) return null;
...
export function selectDefsFiles(project: number, scope = ''): SpecDefsFile[] {
  ...
  for (const file of Object.values(mls.stor.files) as any[]) {
    ...
    // Ignored folders never enter the plan — not as an empty wave, not as a "skipped" line.
    if (waveOf(folder) === null) continue;
...
export function planWaves(project: number, files: SpecDefsFile[], ignoreFreshness = false): SpecWavePlan[] {
  ...
  for (const file of files) {
    const wave = waveOf(file.folder);
    if (wave === null) continue;
    const plan = byWave.get(wave)!;
...
export function planSpecFrontend(prompt: string): SpecPlanResult {
  ...
  if (command.kind === 'target') {
    ...
    if (waveOf(folder) === null) {
      const reason = `agentSpecFrontend: o target esta em uma pasta ignorada (${folder}). Nada foi executado.`;
      return { error: reason, plans: [], report: reason };
    }
    const file: SpecDefsFile = { project, folder, shortName: String(info.shortName) };
...
/** One planId per item: the verify and the repair rounds key findings by it. */
function itemPlanIdOf(item: SpecQueued): string {
  const shortName = item.defPath.split('/').pop()?.replace(/\.defs\.ts$/u, '') ?? '';
  return `spec-item-${safeId(`${item.folder}/${shortName}`)}`;
}

/**
 * One SEQUENTIAL `agentCfeMaterializePhase` step per NON-EMPTY (module, wave), chained in the order
 * module A shared -> module A pages -> module B shared -> ...
 *
 * Why a phase and not the fan-out itself: a `parallel_dynamic` parent starts as soon as it receives
 * its args, so a `dependsOn` on it is ignored (agentCfeCreateScanL4.ts:95) — both waves used to run
 * at once and the pages compiled without their shared. A sequential step does honour `dependsOn`;
 * the phase opens the fan-out only when it runs and stays in_progress until fan-out + verify +
 * repairs are done. The verify is also what reports the MATERIALIZE-FAILED items, which fan-out
 * children never surface on their own (they always complete).
 *
 * One phase per module because the phase keeps only the items of ONE module (itemsInModule) and
...
export function createWaveIntents(context: mls.msg.ExecutionContext, plans: SpecWavePlan[]): mls.msg.AgentIntentAddStep[] {
  ...
  const modules = [...new Set(plans.flatMap(plan => plan.queued.map(item => moduleOf(item.folder))))].sort();
  for (const moduleName of modules) {
    for (const plan of plans) {
      const queued = plan.queued.filter(item => moduleOf(item.folder) === moduleName);
      if (queued.length === 0) continue;
      const planId = `spec-materialize-${safeId(moduleName)}-${plan.wave}`;
      const title = `${WAVE_TITLE[plan.wave]} (${moduleName})`;
      const items = queued.map(item => ({ planId: itemPlanIdOf(item), defPath: item.defPath } satisfies GenStepArgs));
      const step = createAgentStepPayload(
        ...
        {
          planId,
          fanoutPlanId: `${planId}-fanout`,
          title,
          fanoutTitle: `${title} {{completed}}/{{total}}, falhas {{failed}}`,
          items,
        ...
        priorPlanIds,
        'sequential',
        priorPlanIds.length > 0 ? 'waiting_dependency' : 'waiting_human_input',
      );
      intents.push({
        type: 'add-step',
        messageId: '',
