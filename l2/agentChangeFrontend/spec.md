...
> **Atualização 2026-07-15 (create-page):** o fluxo separa `scan-create-l4 -> create-contract-shared-fanout -> create-layout-phase -> create-layout-fanout -> create-reconcile-phase -> reconcile-shared-fanout -> verify-create-layouts -> materialize-create-l2`. Os dois fan-outs dependentes são criados pelos phase steps sequenciais somente depois da barreira anterior; `addParallelArgs` inicia imediatamente um `parallel_dynamic` e por isso não pode ser usado como uma barreira dependente. O scan lê L4 uma vez; cada chamada LLM produz somente um layout para um template pinado. Filhos de fan-out sempre concluem com trace; a validação estrita de `page11` ocorre no gate sequencial posterior.
