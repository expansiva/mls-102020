...
Olhar o `status` dos owners no `l4` e **fazer só o que está pendente** (um "to-be"): criar/atualizar/remover as **telas** + parte **shared** + **contrato BFF** + **layout** da página, chamar a **materialização** (`.defs.ts → .ts`) e registrar no **menu** (`config.json`). No fim, muda o `status`. Pode ser chamado **a qualquer momento** e fazer **um único item** (uma tela, um arquivo). É idempotente: re-rodar só toca no que ainda não está `done`.
...
1. **Varrer** o `l4` em busca de owners (Operations/Workflows) com `status` ≠ `done` — ou processar **um item específico** recebido como argumento.
...
- **Entrada:** `l4` do módulo (owners + status). Opcional: um `operationId`/`workflowId`/página específico para processar só ele.
- **Saída:** páginas/shared/contratos BFF criados/atualizados/removidos em `l2`, materializados em `.ts`, menu atualizado, e `status` dos itens processados atualizado.

## Status (único, no owner)

Pega itens com `status` ≠ `done`; ao iniciar um item seta `inProgress`; ao concluir a parte de frontend, marca `done` **se a mudança for só de frontend**. Se a mudança também exige backend, a convenção é rodar este worker antes do `agentChangeBackend`, e o terminal (backend) marca `done`. *(Detalhe a refinar na implementação — consequência de o status ser único.)*

## O que este agente NÃO faz

- Não decide O QUE muda (isso é `agentChangeSolution`/`agentNewSolution2`); só executa o que já está marcado no `l4`.
- Não gera backend (tabelas, layer_3/4, persistência) — isso é `agentChangeBackend`.
