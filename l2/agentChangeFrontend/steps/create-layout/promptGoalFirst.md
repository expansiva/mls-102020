...
`shared.functions` for state and behavior. Every visible text is referenced by a `titleKey`, `labelKey`
or `emptyKey` — a stable, descriptive dotted key. Every intention includes `fields`, `columns`,
