/// <mls fileReference="_102020_/l2/agentNewSolution3/helpers/ns3Ids.ts" enhancement="_blank"/>
...
export function normalizeNs3Id(value: unknown, fallback = 'item'): string {
...
export function uniqueNs3Id(baseValue: unknown, used: Set<string>, fallback = 'item'): string {
  const base = normalizeNs3Id(baseValue, fallback);
  if (!used.has(base)) {
    used.add(base);
    return base;
  }
  ...
  throw new Error(`[uniqueNs3Id] no available id for ${base}`);
}
...
export function collectDuplicateIds(ids: Iterable<string>): string[] {
  ...
  for (const id of ids) {
    const normalized = normalizeNs3Id(id);
