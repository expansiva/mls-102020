/// <mls fileReference="_102020_/l2/agentNewSolution3/steps/e3-ontology/gate.ts" enhancement="_blank"/>
...
export const E3_MODEL_SCHEMA_VERSION = '2026-07-07-ns3-e3-v1';

export const NS3_ENTITY_KINDS = ['core', 'supporting', 'event', 'metric', 'mdm'] as const;
export const NS3_OWNERSHIPS = ['moduleOwned', 'mdmOwned', 'horizontalOwned', 'pluginOwned', 'existingModuleOwned', 'external'] as const;
export const NS3_SCALAR_FIELD_TYPES = ['uuid', 'string', 'text', 'number', 'money', 'boolean', 'date', 'datetime'] as const;
export const NS3_RELATIONSHIP_TYPES = ['oneToOne', 'oneToMany', 'manyToOne', 'manyToMany', 'partOf'] as const;

export type Ns3EntityKind = typeof NS3_ENTITY_KINDS[number];
export type Ns3Ownership = typeof NS3_OWNERSHIPS[number];
export type Ns3RelationshipType = typeof NS3_RELATIONSHIP_TYPES[number];

export interface Ns3E3ModuleBlock {
  title: string;
  purpose: string;
...
export interface Ns3E3ModelEntity {
  entityId: string;
  title: string;
  description: string;
  kind: Ns3EntityKind;
  ownership: Ns3Ownership;
  statusEnum?: string[];
  lifecycleStates?: string[];
  sourceRefs?: { journeyIds?: string[]; featureIds?: string[] };
}

export interface Ns3E3Relationship {
  relationshipId: string;
  fromEntity: string;
  toEntity: string;
  type: Ns3RelationshipType;
  description: string;
}

export interface Ns3E3ModelArtifact {
  schemaVersion: typeof E3_MODEL_SCHEMA_VERSION;
  moduleName: string;
  userLanguage: string;
  version: number;
  createdAt: string;
  module: Ns3E3ModuleBlock;
  entities: Ns3E3ModelEntity[];
  relationships: Ns3E3Relationship[];
}

export interface Ns3E3Field {
  fieldId: string;
  type: string;
  required: boolean;
  description: string;
...
export interface Ns3E3EntityArtifact {
  entityId: string;
  title: string;
  description: string;
  kind: Ns3EntityKind;
...
export function prepareE3ModelArtifact(input: unknown, context: Pick<E3ModelGateContext, 'moduleName' | 'userLanguage'>): Ns3E3ModelArtifact {
  ...
  return {
    ...
    entities: entities.map(entity => ({
      ...
      kind: readEnum(entity.kind, NS3_ENTITY_KINDS, 'core'),
      ownership: readEnum(entity.ownership, NS3_OWNERSHIPS, 'moduleOwned'),
    ...
    relationships: relationships.map(rel => ({
      ...
      type: readEnum(rel.type, NS3_RELATIONSHIP_TYPES, 'manyToOne'),
      description: readString(rel.description) || '',
    })),
  };
}

export function prepareE3EntityArtifact(input: unknown): Ns3E3EntityArtifact {
  const record = isRecord(input) ? input : {};
  ...
  return {
    ...
    kind: readEnum(record.kind, NS3_ENTITY_KINDS, 'core'),
    ownership: readEnum(record.ownership, NS3_OWNERSHIPS, 'moduleOwned'),
...
export function validateE3ModelInvariants(
  artifact: Ns3E3ModelArtifact,
  context: E3ModelGateContext,
): { artifact: Ns3E3ModelArtifact; issues: Ns3GateIssue[] } {
...
export function validateE3EntityInvariants(
  entity: Ns3E3EntityArtifact,
  context: E3EntityGateContext,
): { artifact: Ns3E3EntityArtifact; issues: Ns3GateIssue[] } {
  const issues: Ns3GateIssue[] = [];
  ...
  for (const field of entity.fields) {
    ...
    const isScalar = (NS3_SCALAR_FIELD_TYPES as readonly string[]).includes(field.type);
    if (!isScalar && !knownEntityIds.has(field.type)) {
      issues.push(errorIssue('field.type.invalid', `entity ${entity.entityId}: field ${field.fieldId} type "${field.type}" is neither a scalar (${NS3_SCALAR_FIELD_TYPES.join('|')}) nor a declared entity`, field.fieldId));
    }
