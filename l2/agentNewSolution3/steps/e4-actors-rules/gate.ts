/// <mls fileReference="_102020_/l2/agentNewSolution3/steps/e4-actors-rules/gate.ts" enhancement="_blank"/>
...
import { lowerFirst, toPascalCase } from '/_102020_/l2/agentNewSolution3/steps/e3-ontology/gate.js';

export const E4_SCHEMA_VERSION = '2026-07-07-ns3-e4-v1';

export const NS3_RULE_LAYERS = ['domain', 'application'] as const;
export type Ns3RuleLayer = typeof NS3_RULE_LAYERS[number];

export interface Ns3E4Actor {
  actorId: string;
  title: string;
  description: string;
...
export interface Ns3E4ExternalRef {
  title: string;
  reason: string;
}

export interface Ns3E4ExternalRefs {
  mdm: Ns3E4ExternalRef[];
  horizontals: Ns3E4ExternalRef[];
  plugins: Ns3E4ExternalRef[];
  agents: Ns3E4ExternalRef[];
}

export interface Ns3E4Artifact {
  ...
  actors: Ns3E4Actor[];
  rules: Ns3E4Rule[];
  externalRefs: Ns3E4ExternalRefs;
}
...
export function prepareE4Artifact(input: unknown, context: { moduleName: string; userLanguage: string }): Ns3E4Artifact {
  ...
  return {
    ...
    rules: rules.map(rule => ({
      ...
      layer: (readString(rule.layer) || 'domain') as Ns3RuleLayer,
...
export function validateE4Invariants(
  artifact: Ns3E4Artifact,
  context: E4GateContext,
): { artifact: Ns3E4Artifact; issues: Ns3GateIssue[] } {
...
export function renderE4Markdown(artifact: Ns3E4Artifact, options: { generatedAt?: string } = {}): string {
  const lines: string[] = [];
  ...
  lines.push('');
  const groups: Array<[string, Ns3E4ExternalRef[]]> = [
...
function readRefArray(value: unknown): Ns3E4ExternalRef[] {
