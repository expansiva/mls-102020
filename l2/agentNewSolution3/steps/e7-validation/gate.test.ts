/// <mls fileReference="_102020_/l2/agentNewSolution3/steps/e7-validation/gate.test.ts" enhancement="_blank"/>
...
import { Ns3E3EntityArtifact, Ns3E3ModelArtifact } from '/_102020_/l2/agentNewSolution3/steps/e3-ontology/gate.js';
import {
  buildNs3ModuleDefs,
  buildNs3TodoOwners,
...
function validE2(): Ns3E2JourneysArtifact {
  return {
    schemaVersion: '2026-07-06-ns3-e2-v1',
    moduleName: 'cafeFlow',
...
function validModel(): Ns3E3ModelArtifact {
  return {
    schemaVersion: '2026-07-07-ns3-e3-v1',
    moduleName: 'cafeFlow',
    userLanguage: 'en',
    version: 1,
    createdAt: '2026-07-07T00:00:00.000Z',
...
function validEntities(): Ns3E3EntityArtifact[] {
  return [
    {
      entityId: 'Order',
      title: 'Order',
...
function validInput(): Ns3E7HealthInput {
  return {
    moduleName: 'cafeFlow',
    e2: validE2(),
    model: validModel(),
...
void test('e7 health report passes on a consistent module', () => {
  const report = computeNs3HealthReport(validInput());
...
void test('e7 flags a workflow referencing an unknown operation', () => {
  ...
  const report = computeNs3HealthReport(input);
...
void test('buildNs3TodoOwners produces one toCreate owner per behavior with the correct defPath', () => {
  const input = validInput();
  const owners = buildNs3TodoOwners({ moduleName: 'cafeFlow', workflowDefs: input.workflowDefs, operationDefs: input.operationDefs });
  ...
  assert.equal(workflowOwner.defPath, 'l4/workflows/takeoutOrderLifecycle.defs.ts');
  ...
  assert.equal(operationOwner.defPath, 'l4/operations/createOrder.defs.ts');
...
void test('buildNs3ModuleDefs carries the module block, relationships and approvedArtifacts', () => {
  const input = validInput();
  const externalRefs = {
  ...
  const moduleDefs = buildNs3ModuleDefs({
    moduleName: 'cafeFlow',
    model: input.model,
