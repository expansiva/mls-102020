/// <mls fileReference="_102020_/l2/agentNewSolution3/steps/e7-validation/gate.ts" enhancement="_blank"/>
...
export interface Ns3E7ClassificationWorkflow {
  workflowId: string;
  actorId: string;
...
export interface Ns3E7ClassificationOperation {
  operationId: string;
  actorId: string;
  entity: string;
  kind: string;
...
export interface Ns3E7Classification {
  workflows: Ns3E7ClassificationWorkflow[];
  operations: Ns3E7ClassificationOperation[];
}

// Loose shapes for the defs saved by E5/E6: E7 only reads the fields it checks.
export interface Ns3E7WorkflowDef {
  workflowId?: string;
  title?: string;
  pageId?: string;
  actor?: string;
...
export interface Ns3E7OperationDef {
  operationId?: string;
  title?: string;
  actor?: string;
  entity?: string;
...
export interface Ns3E7Workspace {
  workspaceId: string;
  operationIds: string[];
  actor: string;
  workflowId?: string;
...
export interface Ns3E7JourneyMap {
  workspaces: Ns3E7Workspace[];
  landings: unknown[];
}

export interface Ns3E7HealthInput {
  moduleName: string;
  e2: Ns3E2JourneysArtifact;
  model: Ns3E3ModelArtifact;
  entities: Ns3E3EntityArtifact[];
  e4: { actors: { actorId: string }[]; rules: { ruleId: string }[] };
  classification: Ns3E7Classification;
  workflowDefs: Ns3E7WorkflowDef[];
  operationDefs: Ns3E7OperationDef[];
  journeyMap: Ns3E7JourneyMap | null;
}

export interface Ns3E7HealthReport {
  passed: boolean;
...
export interface Ns3E7ExternalRefs {
  mdm: unknown[];
  horizontals: unknown[];
  plugins: unknown[];
  agents: unknown[];
...
export interface Ns3E7ModuleDefsArgs {
  moduleName: string;
  model: Ns3E3ModelArtifact;
  entities: Ns3E3EntityArtifact[];
  e1Draft: Record<string, unknown> | null;
  e2: Ns3E2JourneysArtifact;
  e4ExternalRefs: Ns3E7ExternalRefs;
  journeyDefPath: string;
}

export interface Ns3E7ModuleDecision {
...
export interface Ns3E7OntologyIndexEntry {
  title: string;
  description: string;
  kind: string;
  ownership: string;
...
export interface Ns3E7ModuleDefs {
  ...
  designContext: {
    ...
    decisions: Ns3E7ModuleDecision[];
  };
  ontology: { entities: Record<string, Ns3E7OntologyIndexEntry> };
  journey: { defPath: string };
  relationships: Ns3E3Relationship[];
  approvedArtifacts: Ns3E7ExternalRefs;
}

export interface Ns3E7TodoOwner {
...
export interface Ns3E7NextStep {
  id: string;
  title: string;
  description: string;
}
...
export function computeNs3HealthReport(input: Ns3E7HealthInput): Ns3E7HealthReport {
  ...
  for (const id of classifiedWorkflowIds) {
    if (!workflowDefIds.has(id)) issues.push(errorIssue('plan.disk.divergence', `classified workflow '${id}' has no def on disk (l4/workflows/${id}.defs.ts)`));
  }
  ...
  for (const id of classifiedOperationIds) {
    if (!operationDefIds.has(id)) issues.push(errorIssue('plan.disk.divergence', `classified operation '${id}' has no def on disk (l4/operations/${id}.defs.ts)`));
  }
  ...
  if (!input.journeyMap) {
    issues.push(errorIssue('journey.missing', `journey map defs not found (l4/${input.moduleName}/journeys/${input.moduleName}Journeys.defs.ts)`));
  } else {
...
export function buildNs3ModuleDefs(args: Ns3E7ModuleDefsArgs): Ns3E7ModuleDefs {
  ...
  const decisions: Ns3E7ModuleDecision[] = (args.e2.decisions || []).map(decision => {
  ...
  const entities: Record<string, Ns3E7OntologyIndexEntry> = {};
...
export function buildNs3TodoOwners(args: {
  moduleName: string;
  workflowDefs: Ns3E7WorkflowDef[];
  operationDefs: Ns3E7OperationDef[];
}): Ns3E7TodoOwner[] {
  const owners: Ns3E7TodoOwner[] = [];
  for (const def of args.workflowDefs) {
    ...
    owners.push({
      ...
      status: 'toCreate',
      defPath: `l4/workflows/${workflowId}.defs.ts`,
  ...
  for (const def of args.operationDefs) {
    ...
    owners.push({
      ...
      status: 'toCreate',
      defPath: `l4/operations/${operationId}.defs.ts`,
      pageId,
      commandName,
...
export function renderE7Markdown(
  report: Ns3E7HealthReport,
  options: { moduleName: string; nextSteps: Ns3E7NextStep[] },
): string {
  const lines: string[] = [];
  ...
  lines.push('- full machine report: `l4/trace/behavior-health-report.json`');
  lines.push('');
...
function collectDefActors(def: Ns3E7WorkflowDef): string[] {
