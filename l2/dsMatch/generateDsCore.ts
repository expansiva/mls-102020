...
// (agents/designSystem/agentGenerateDs.ts) sends { name?, description?, palette?, brief? } and
// the LLM answers a compact map of COLOR FAMILY BASES (11 families × {light, dark}). This module
// deterministically EXPANDS those bases into the full mandatory token set (lighter/darker/light/
// dark variants × hover/focus/disabled states + `_dark-` pairs) via HSL math, and fills
// global/typography from the canonical template (`_102029_` — single source of truth). No
// storage/framework imports — testable in isolation.
//
...
// produces a complete, valid entry (missing/invalid family → template default for that family).
...
export interface GenerateDsRequest {
    projectId: number;
    palette?: string[];          // brand colors — the SOURCE the LLM maps to family bases
...
/** Sanitized generation output — the tokens portion the plugin loads into its Add form. */
...
/** '#abc' / '#A1B2C3' → '#aabbcc' (lowercase, 6-digit); anything else → null. */
...
// ─── Family → mandatory keys expansion ──────────────────────────────────────────

// Lightness deltas (percentage points) that turn a family BASE into each variant/state.
// Tuned to yield distinct, readable shades — not to reproduce the template's hand-authored
...
export function parseColorKey(key: string): ParsedColorKey | null {
    ...
    let tail = rest.slice(`${family}-color`.length); // '' | '-lighter' | '-darker-hover' | …
    let state = '';
...
export function buildGenerateDsHumanPrompt(req: GenerateDsRequest): string {
    ...
    if (req.palette?.length) {
        parts.push([
            `## Brand palette (SOURCE — map these to the family bases)`,
...
// ─── Sanitization (deterministic — the LLM output never reaches the entry unchecked) ─
 ...
 * - families: each valid #hex light/dark pair seeds its family expansion; invalid/missing → default.
