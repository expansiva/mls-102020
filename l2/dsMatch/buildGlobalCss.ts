 ...
 */
export function tokensToCss(tokens: DsTokens): string {
...
/** Build the full stylesheet string for ONE design system's tokens. */
export function renderGlobalCss(tokens: DsTokens): string {
...
export async function buildGlobalCss(project: number, ds?: number | string): Promise<void> {
    ...
    for (const k of keys) {
        ...
        await writeFileByRef(dsGlobalCssRef(project, k), renderGlobalCss(tokens));
    }
}
