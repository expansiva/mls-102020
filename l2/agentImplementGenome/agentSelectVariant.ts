...
// agentSelectGroups (which wrote `groupSelections`).
//
...
async function afterPromptStep(
  ...
  try {
    ...
    return [mkFail(context, parentStep, step, hookSequential, `[agentSelectVariant] ${error instanceof Error ? error.message : String(error)}`)];
  }
}

/** The "new defs": resolved molecule assignments (LLM-picked) for agentGenDefs to weave. */
function buildAssignmentsDefs(defsRef: string, organisms: unknown, usagePaths: string[]): string {
  ...
  return [
    ...
    '',
    '// Generated by agentSelectVariant (Agent2). Variant chosen by the LLM from the',
    '// DS-compatible candidates, using the page context. Consumed by agentGenDefs.',
    '',
