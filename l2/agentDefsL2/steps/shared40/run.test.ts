...
void test('tampered persisted contract cannot produce an approved shared result or barrier', async () => {
  ...
  await assert.rejects(() => persistD2SharedUnit(IDENTITY, host.snapshot, PAGE, sharedSource(), 1), /D2_SHARED_CONTRACT_HASH_MISMATCH/);
  ...
  await persistD2SharedUnit(IDENTITY, host.snapshot, PAGE, sharedSource(), 1);
...
void test('snapshot switched after shared write cannot produce an approved result or barrier', async () => {
  ...
  await assert.rejects(() => persistD2SharedUnit(IDENTITY, host.snapshot, PAGE, sharedSource(), 1, verify), /D2_SHARED_STALE_RUN/);
...
async function installHost() {
  ...
  (globalThis as unknown as { mls: unknown }).mls = { actualProject: IDENTITY.project, stor: { files, getKeyToFile: keyOf, localStor: { setContent: async (file: Stored, value: { content: string }) => { file.content = value.content; } } } };
  return { files, snapshot, contractSource, seed };
}
