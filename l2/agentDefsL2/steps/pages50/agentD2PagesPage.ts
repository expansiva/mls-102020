...
export function createAgent(): IAgentAsync { return { agentName: D2_PAGES_PAGE_AGENT_NAME, agentProject: 102020, agentFolder: 'agentDefsL2/steps/pages50', agentDescription: 'Describe desktop and mobile presentations for one page with one bounded repair', visibility: 'private', beforePromptStep, afterPromptStep }; }
...
export async function beforePromptStep(_agent: IAgentMeta, context: mls.msg.ExecutionContext, parentStep: mls.msg.AIAgentStep, step: mls.msg.AIAgentStep, hookSequential: number, args?: string): Promise<mls.msg.AgentIntent[]> {
  try {
    const parsed = parseArgs(args || step.prompt || ''); const identity = { project: parsed.project, module: parsed.module };
...
export async function afterPromptStep(_agent: IAgentMeta, context: mls.msg.ExecutionContext, parentStep: mls.msg.AIAgentStep, step: mls.msg.AIAgentStep, hookSequential: number): Promise<mls.msg.AgentIntent[]> {
  ...
  try {
    const judgment = parseD2PagesJudgment(unwrap(step.interaction?.payload?.[0]));
    const snapshot = await readD2Input(identity); if (!snapshot) throw new Error('D2_PAGES_INPUT_MISSING');
...
function unwrap(value: unknown): unknown { const root = record(value); const candidate = root.arguments ?? root.payload ?? root; if (typeof candidate !== 'string') return candidate; try { return JSON.parse(candidate); } catch { throw new Error('D2_PAGES_SCHEMA_TRUNCATED'); } }
