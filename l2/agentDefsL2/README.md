...
Current delivery implements `entry10`, deterministic `input20`, deterministic `contracts30` and
per-page LLM `shared40` with one bounded repair.
...
pipeline items without materializing runtime `.ts`; `pages50` and `finalize60` remain unavailable.
